...
################################
#########GeneBody bedfile for enhancer overlap#####
...
bedfile = read.delim("/workspace/rsrch2/common_data/Refgenome/mm10/GenCode.vM25.mm10.Gene_bed/GenCodeVM25.gene.geneBodyUPDn100kb.padded.filtered.bed",header = F)
names(bedfile)=c("Chr","Start","End","Gene","Score","Strand")


bed_res <- lapply(8:12,function(x){
  genes=na.omit(genelist[,x])
  befile_file = bedfile %>% filter(Gene %in% genes)
  filename = paste0("./LostRhyInCase/Lost_Enhancer.BedFiles/",names(genelist)[x],".bed")
